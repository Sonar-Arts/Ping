...
class LevelViewWidget(QWidget):
    ...
    def update_pygame(self):
        ...
        for obj_data in self.core_logic.level_objects:
            ...
            # Handle sprites based on image_path property
            ...
            if image_path:
                ...
                # Skip loading if we know this sprite had an error before
                if image_path in self.sprite_load_errors:
                    draw_placeholder = True
                    placeholder_color = (255, 0, 0)  # Red for failed sprites
                    continue
                    ...
                    try:
                        ...
                        if pygame.get_init():
                            try:
                                ...
                                if sprite.get_size() != (draw_w, draw_h):
                ...
                if not sprite_to_draw:
                    draw_placeholder = True
                    placeholder_color = (200, 0, 0)  # Red for missing sprite


            # Draw the object
            if sprite_to_draw:
                self.pygame_surface.blit(sprite_to_draw, rect.topleft)
            elif final_blit_surface:
                self.pygame_surface.blit(final_blit_surface, rect.topleft)
            elif draw_placeholder:
                pygame.draw.rect(self.pygame_surface, placeholder_color, rect)
                try:
                    if pygame.font.get_init():
                        font_size = max(10, min(rect.width, rect.height) // 2)
                        if font_size > 0:
    ...
    def _place_object_at(self, obj_type_key, x, y):
        ...
        if width is not None and height is not None:
            ...
            new_obj_data['height'] = obj_h
             ...
             new_obj_data['height'] = obj_h
             # Handle sprite path initialization
             image_path = defaults.get('image_path', '')
             new_obj_data['image_path'] = image_path
             
             # Will be added to sprite_paths after core_logic assigns an ID
             # Track sprite usage in _handle_object_update when we get the update signal
    ...
    def _handle_object_update(self, obj_id):
        ...
        if not obj_data:
            # Object was deleted - clean up its sprite associations
            if obj_id in self.sprite_paths:
                old_path = self.sprite_paths.pop(obj_id)
                # Check if any other objects still use this sprite
        ...
        # Handle sprite path changes for existing objects
        if obj_data.get('type') == 'sprite':
            new_path = obj_data.get('image_path')
            old_path = self.sprite_paths.get(obj_id)
            
            if old_path != new_path:
                # Remove old sprite path association
                if old_path:
                    self.sprite_paths.pop(obj_id)
                    # Check if any other objects still use this sprite
                    if not any(path == old_path for path in self.sprite_paths.values()):
                        if old_path in self.sprite_cache:
                            del self.sprite_cache[old_path]
                            print(f"Removed unused sprite '{old_path}' from cache")
                ...
                # Add new sprite path association
                if new_path:
                    self.sprite_paths[obj_id] = new_path
                    # Cache will be populated on next draw if needed
